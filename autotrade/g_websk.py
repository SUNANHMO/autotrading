...
class WebSocketClient:
	...
	async def receive(self):
		try:
				...
				if response.get("trnm") == "LOGIN":
					if response.get("return_code") == 0:
						...
						if state["code"]:
							...
							print(f"Re-register price: {self.current_code}")
							await self.register_price(self.current_code)
					elif response.get("return_code") in (805004, 8005):
						...
						if new_token:
							...
							print("Failed to issue new token")
						await self.disconnect()
						return
					else:
						print("LOGIN FAIL:", response)
						log("★WEBSK_receive")
						await self.disconnect()
						return
			...
			log(f"★WEBSK_receive {type(e).__name__}: {e}")
	...
	async def run(self):
		while self.running:
			try:
				...
				print("run receive")
				# 로그인 성공 후 receive() 시작
				# 기존 종목 재등록은 LOGIN 성공 직후 receive()에서 처리
				await self.receive()
		...
		print("WebSocket run loop stopped")
	...
	async def register_price(self, code):
		...
		print(f"REGISTER PRICE START : {code}")
		self.price_data["current_price"] = 0
		self.price_data["expected_price"] = 0
		await self.send({
			"trnm": "REG",
			"grp_no": "1",
			"refresh": "1",
	...
	async def reconnect(self):
		print("WEBSK reconnect requested")
		self.search_result = None
		await self.disconnect()
		print("WEBSK reconnect signal sent")
	def handle_real(self, response):
		for item in response.get("data", []):
			...
			if real_type == "0B":
				...
				price = abs(int(values.get("10") or 0))
				#print("0H RECEIVED:",self.current_code,"price=",price,"raw=",values)
				self.price_data["expected_price"] = price
