...
class WebSocketClient:
	...
	async def receive(self):
		try:
				...
				if response.get("trnm") == "LOGIN":
					if response.get("return_code") == 0:
						...
						if response.get("return_code") == 8005:
							print("토큰 만료 → 새 토큰 발급") #토큰 만료 → 새 토큰 발급
			...
			print("웹소켓 연결 종료") #웹소켓 연결 종료
			self.connected = False

		except Exception as e:
			print(f"웹소켓 오류: {e}") #웹소켓 오류
			self.connected = False
	async def run(self):
		while True:
			try:
			...
			if real_type == "0B":
				...
				if (
					...
					if remain_qty == 0:
						...
						save_state(state)
						log(f"S_{avg_price}") #매도 체결 완료 (평균 {avg_price}원)
