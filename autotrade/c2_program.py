...
#계좌관리
def get_account_info(token, date):

	params = {"qry_dt": date,}
	result, _, _ = fn_ka01690(token=token,data=params)
...
def get_asset(token=None, date=None, account=None):
	...
	return int(account["day_stk_asst"])
def get_holding(token=None, date=None, account=None):
	...
	holdings = account.get("day_bal_rt", [])
