...
def is_370day_max(program_data, code):
	...
	print(code, "Data Count:", len(data))
	# 오늘 실제 값
	raw_today = parse_amount(data[0]["prm_netprps_amt"])
	# 오늘이 음수면 매수 대상이 아님
	# 절대값 최대이더라도 음수 최대는 매수하지 않음
	if raw_today <= 0:
		print(code, "370-Day Max but Negative")
		return False
	# 오늘은 양수이므로 절대값 기준으로 최대 비교
	today = abs(raw_today)
	# 과거 370일과 비교
	for item in data[1:]:
		value = abs(parse_amount(item["prm_netprps_amt"]))
		if value >= today:
			print(code, "370-Day Disqualified")
			return False
	print(code, "370-Day Passed")
	return True
def is_first_after_crossup(token, code, program_data_500, date):
	...
	for i in range(len(data)):
		# 40일선 30% 돌파 이전 데이터는 제외
		if data[i]["dt"] < cross_date:
			continue
		# 오늘 실제 프로그램 순매수금액
		raw_value = parse_amount(
			data[i]["prm_netprps_amt"])
		# 절대값으로 최대 여부 판단
		today_value = abs(raw_value)
		# 직전 370일 범위
		start = max(0, i - 370)
		# 직전 370일의 절대값 최대
		max_value = 0
		for j in range(start, i):
			...
			if value > max_value:
				max_value = value
		# ---------------------------------------
		# 이 날이 직전 370일 절대값 최대가 아니면
		# 아무것도 하지 않고 다음 날짜로 이동
		# ---------------------------------------
		if today_value <= max_value:
			continue
		# ---------------------------------------
		# 여기까지 왔다는 것은
		# "370일 절대값 최대"가 발생했다는 뜻
		# ---------------------------------------
		# ---------------------------------------
		# 중요:
		# 음수 최대는 "최초 매수 이벤트"로 인정하지 않음
		# ---------------------------------------
		if raw_value <= 0:
			print(
				code,
				"370-Day Absolute Max but Negative:",
				data[i]["dt"],
				raw_value)
			# 음수 최대는 무시하고
			# 이후 날짜를 계속 검사
		...
		# "양수이면서 370일 절대값 최대"
		# ---------------------------------------
		# 과거에 이미 양수 최대가 발생했다는 뜻
		# → 오늘은 두 번째 이후 최대이므로 매수하지 않음
		if i != len(data) - 1:
			print(
				code,
				"First Positive 370-Day Max After Breakout "
				"Already Occurred:",
				data[i]["dt"])
			return False
		# ---------------------------------------
		...
		# → 매수 가능
		# ---------------------------------------
		print(code,"First Positive 370-Day Max After Breakout")
		return True
	print(code, "Conditions Not Met")
	return False
...
def find_take_profit_order(token, code):
	...
	for item in result["oso"]:
		if (
			...
			order_no = item["ord_no"]
			profit_market = item["stex_tp_txt"]
			print(
				f"미체결 익절 주문 발견: "
				f"주문번호={order_no}, "
				f"거래소={profit_market}, "
				f"미체결수량={item['oso_qty']}")
			return order_no, profit_market
	return None, None