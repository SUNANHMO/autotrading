	...
	while True:
		try:
			...
			if current >= TOKEN_TIME and not executed[TOKEN_TIME]:
				...
				log("[07:00] 거래 준비 시작")
				summary = get_account_summary(token) # 계좌 조회
				state = load_state()				 # state 조회
				if summary["has_holding"]:
					...
					clear_state()
					state = load_state()
				log(f"예수금 : {summary['cash']}")
				log(f"추정자산 : {summary['asset']}")
				log(f"보유여부 : {summary['has_holding']}")
				log("[07:00] 거래 준비 완료")
			...
			if current >= SEARCH_TIME and not executed[SEARCH_TIME]:
				...
				log("[15:20:05] 종목 검색 시작")
				buy_code = get_buy_code(token)
				if buy_code is None:
					log("매수 후보 없음")
				else:
					state = load_state()
			...
			if websocket and MONITOR_START_TIME <= current < MONITOR_END_TIME:
				current_price = websocket.price_data["current_price"]
				if current_price > 0:
					state = load_state()
					if state["holding"]:
						# 이미 시장가 매도 주문이 접수되어 있으면 다시 주문하지 않음
						if state["sell_order_no"] is None:
							stop_price = int(state["buy_price"] * (1 - abs(STOP_LOSS_RATE)))
							if current_price <= stop_price:
								log("[손절] 조건 충족")
								order_no = market_sell(token)
								if order_no:
									log(f"시장가 매도 주문 : {order_no}")

			await asyncio.sleep(1)

		except Exception as e:
			log(f"Scheduler 오류 : {e}")
