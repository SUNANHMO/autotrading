...
def sync_state(token):
	...
	# state와 동일하면 아무것도 안 함
	if state["holding"] and state["code"] == stock["code"]:
		return
	...
	state["take_profit_order_no"] = find_take_profit_order(token,stock["code"])
	state["profit_market"] = None
	state["sell_order_no"] = None

	buy_date = find_buy_date(token, stock["code"])
	if buy_date:
