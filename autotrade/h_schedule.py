	...
	while True:
		try:
			...
			if current >= TOKEN_TIME and not executed[TOKEN_TIME]:
				...
				print("[07:00] 거래 준비 시작") #[07:00] 거래 준비 시작
				...
				print(f"추정자산 : {summary['cash']}") #추정자산
				print(f"매입금액 : {summary['asset']}") #매입금액
				print(f"보유여부 : {summary['has_holding']}") #보유여부
				print("[07:00] 거래 준비 완료")
			...
			if current >= NXT_TAKE_PROFIT_TIME and not executed[NXT_TAKE_PROFIT_TIME]:
				...
				state = load_state()
				if state["holding"]:
					print("[08:00] NXT 익절주문") #[08:00] NXT 익절주문
			...
			if current >= FORCE_SELL_TIME and not executed[FORCE_SELL_TIME]:
				...
				if should_force_sell(token):
					...
					print(f"market_sell 반환값 : {order_no}")
					if order_no:
						print("강제청산 주문")
