...
from c2_program import find_buy_date, find_take_profit_order
...
def sync_state(token):
	...
	# ---------------------------------
	# 1. 계좌에 보유종목이 없으면 state 초기화
	# ---------------------------------
	if not summary["has_holding"]:
		clear_state()
		return
	...
	# 3. 실제 매수일 복구
	# ---------------------------------
	# 기존 state에 buy_date가 있으면 유지
	# 없으면 거래내역을 조회해서 실제 매수일 찾기
	if not state.get("buy_date"):
		buy_date = find_buy_date(
			token,
			stock["code"]
		)
		...
		if buy_date:
			...
			print("BUY DATE NOT FOUND")
			state["buy_date"] = None

	# ---------------------------------
	# 4. 실제 미체결 익절 주문 복구
	# ---------------------------------
	take_profit_order_no, profit_market = find_take_profit_order(
		token,
		stock["code"]
	)

	state["take_profit_order_no"] = take_profit_order_no
	state["profit_market"] = profit_market

	# ---------------------------------
	# 5. 매수 대기 주문은 이미 보유 중이므로 초기화
	# ---------------------------------
	...
	# ---------------------------------
	# 현재 미체결 익절 주문이 있다면
	# take_profit_order_no에 저장되어 있으므로
	# sell_order_no는 초기화
	...
	while True:
		try:
			...
			if (
				...
				if time.time() - websocket.last_alive > 60:
					log("★WEBSK_ERROR")
			...
			if (TOKEN_TIME <= current < NXT_TAKE_PROFIT_TIME and not executed[TOKEN_TIME]):
				...
				if summary["has_holding"]:
					...
					if not state.get("buy_date"):
						state["buy_date"] = datetime.today().strftime("%Y%m%d")
					save_state(state)
			...
			if (MONITOR_START_TIME <= current < SEARCH_TIME and not executed[MONITOR_START_TIME]):
				...
				sync_state(token)
				state = load_state()
				if state["holding"] and state["take_profit_order_no"] is None:
					order_no = find_take_profit_order(token, state["code"])
					if order_no:
			...
			# 15:15 시스템 점검
			if (AFTERNOON_CHECK_TIME <= current < FORCE_SELL_TIME
					and not executed[AFTERNOON_CHECK_TIME]):
				executed[AFTERNOON_CHECK_TIME] = True
				# ==========================================
				# 15:15 토큰 강제 갱신
				# ==========================================
				print("15:15 Token Refresh")
				new_token = get_token()
				if new_token:
					token = new_token
					print("15:15 Token Refresh Success")
					if websocket is not None:
						websocket.token = token
						await websocket.reconnect()
						# 새 웹소켓 LOGIN 완료까지 잠시 대기
						for _ in range(50):
							if websocket.connected:
								break
							await asyncio.sleep(0.1)
					...
					log("★TOKEN_REFRESH")
					print("15:15 Token Refresh Failed")
				# ==========================================
				# 15:15 시스템 점검
				# ==========================================
			...
			# 15:29:59 매수
			if (BUY_TIME <= current < CANCEL_TAKE_PROFIT_TIME
					and not executed[BUY_TIME]):
				executed[BUY_TIME] = True
				token, websocket = await ensure_websocket(token, websocket)
				if websocket is None:
					log("★WEBSK_BUY_TIME")
					continue
				# 여기서는 sync_state() 호출하지 않음
				state = load_state()
				if state["holding"]:
					log("SKIP_BUY")
					...
					print(
						"BUY CHECK:",
						"code=", state.get("code"),
						"current_code=", websocket.current_code,
						"expected_price=", websocket.price_data.get("expected_price"))
			...
			if (CANCEL_TAKE_PROFIT_TIME <= current < "20:00:00" and not executed[CANCEL_TAKE_PROFIT_TIME]):
				...
				token, websocket = await ensure_websocket(token, websocket)
				if websocket is None:
					log("★WEBSK_CANCEL_TAKE_PROFIT_TIME")
					continue
				state = load_state()
				# 익절 주문 취소
				...
				# ---------------------------------
				# 하루 작업 종료 → WebSocket 종료
				# ---------------------------------
				if websocket is not None:
					await websocket.stop()
					websocket = None
					print("WebSocket stopped for today")
