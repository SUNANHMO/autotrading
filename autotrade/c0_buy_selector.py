...
def get_buy_code(token, codes):
	...
	for code in codes:
		...
		if is_short_overheat_warning(day_data, listed_shares):
			print(f"Excluded (Short-term Overheating): {code}")  # 단기과열(예고) 제외
			continue
		...
		print(f"Candidate Selected: {code}")  # 후보 선정

	# 최종 후보 종목 선정 및 출력
	if candidate:
		buy_code = min(candidate, key=lambda code: get_market_cap(token, code, test_date))
		print(f"Final Buy Stock: {buy_code}")  # 최종 매수종목
		return buy_code

	print("No Final Candidates Found")  # 최종 후보 없음
	return None